...
export class AuthUtils
{
    ...
    static isTokenExpired(token: string, offsetSeconds?: number): boolean
    {
        ...
        if ( date === null )
        {
            console.log('Failed to get token expiration date');
            return true;
        }

        const tglSekarang = new Date().valueOf();
        const tokenHabis =  date.valueOf() + offsetSeconds * 1000;

        // console.log('Current date = ' + tglSekarang);
        // console.log('Token expired at = ' + tokenHabis);


        if(tglSekarang > tokenHabis){
            return true;
        }
    }
